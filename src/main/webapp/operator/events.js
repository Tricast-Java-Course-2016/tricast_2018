window.onload = function() {
    ...
    const readCustomInputsFrom = function(form){
    	...
    	$.each($(form).find('[data-custom-input][data-name][data-input-type]'), function(index, elem){
    		...
    		switch (elem.attr('data-input-type').toLowerCase()) {
				...
				case 'date-time':
					data[elem.attr('data-name')] = moment(elem.find('[type="date"]').val() + ' ' + elem.find('[type="time"]').val()).toISOString();
					break;
    ...
    $.when( 
	    ...
	    SB.Utils.getAjax('/sportsbook/api/periodtypes', false)
	).done(function(sports, leagues, competitors, events, periods){
		...
		$('#period-market-type-table').html( Handlebars.compile( $('#period-market-type-table-template').html() )({
			marketTypes: [
		...
		$('#new-event-form select[name="leagueId"]').on('change', function(){
			...
			$.each($('#new-event-form [data-name="competitorIds"] option'), function(index, option){
				...
				if(leagueId === -1)
					...
					option.prop('disabled', competitorsById[option.attr('value')].leagueIds.indexOf(leagueId) === -1);
			});
			$.each($('#period-market-type-table tr'), function(index, tr){
				...
				if(sportId === -1)
					...
					tr.find('input[type="checkbox"]').prop('disabled', tr.attr('data-sport-id') != sportId);
			});
		});
		...
		$('#new-event-form').on('submit', function(e){
			...
			const data = Object.assign({}, SB.Utils.readFormData($('#new-event-form')), readCustomInputsFrom($('#new-event-form'))); 
			SB.Utils.postAjax('/sportsbook/api/events', data, false, function(){
