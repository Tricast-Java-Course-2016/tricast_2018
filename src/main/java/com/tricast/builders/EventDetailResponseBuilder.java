...
public class EventDetailResponseBuilder {
    ...
    public static EventDetailResponse build(Long id,
    	...
    	for(int i=0;i<marketRepository.findByEvent(eventRepository.findById(id)).size();i++) {
    		currentPeriodMarkets=new PeriodMarkets();
    		currentPeriodMarkets.setMarket(marketRepository.findByEvent(eventRepository.findById(id)).get(i));
    		currentPeriodMarkets.setPeriod(marketRepository.findByEvent(eventRepository.findById(id)).get(i).getPeriodTypeId());
    		
    		currentPeriodMarkets.setOutcomes(outcomeRepository.findByMarket(marketRepository.findByEvent(eventRepository.findById(id)).get(i)));	
